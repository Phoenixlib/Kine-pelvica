    ...
    >
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="min-w-0">
          <h4 className="font-subtitle font-bold text-teal text-sm truncate">
            {srv.name}
          </h4>
          {srv.description && (
            <p className="text-xs text-teal/70 font-body line-clamp-1 mt-0.5">
              {srv.description}
            </p>
          )}
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3 shrink-0">
        ...
        <span
          className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[8px] font-subtitle font-bold uppercase tracking-wider ${
            srv.isActive
      ...
      <div className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-cream/20 bg-[#faf6f2]/85">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2.5 min-w-0 flex-wrap">
            <h3 className="font-title text-lg text-teal font-semibold truncate">
              {cat.name}
            </h3>
            ...
            <span
              className={`shrink-0 inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[8px] font-subtitle font-bold uppercase tracking-wider ${
                cat.isActive
                  ? "bg-emerald-50 text-emerald-700 border border-emerald-200"
                  : "bg-redbrown/10 text-redbrown border border-redbrown/20"
              }`}
      ...
      <div className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-cream/20 bg-offwhite/50">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            <h3 className="font-title text-lg text-teal/70 font-semibold truncate italic">
              Sin Categoría
            </h3>
...
export default function ServiciosPage() {
  ...
  const { data: services, isLoading: servicesLoading } =
    api.service.getAllAdmin.useQuery();
  const { data: categories, isLoading: categoriesLoading } =
    api.service.getCategories.useQuery();
      ...
      if (activeCatId === overCatId) {
        if (activeId !== overId) {
          setLocalServices((prev) => {
            ...
            const oldIndexInCat = catServices.findIndex(
              (s) => s.id === activeId,
            );
            const newIndexInCat = catServices.findIndex((s) => s.id === overId);

            const reorderedCatServices = arrayMove(
              catServices,
              oldIndexInCat,
              newIndexInCat,
            );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-title text-3xl text-teal">
            Servicios y Categorías
          </h1>
          <p className="font-body text-sm text-teal/70 mt-1">
            Administra tus terapias clínicas, ordénalas arrastrándolas y
            sincronízalas automáticamente con Cal.com.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        (groupedServices["uncategorized"]?.length || 0) === 0 ? (
        <div className="bg-white p-12 rounded-3xl border border-cream/30 text-center text-teal/50 font-medium shadow-xs">
          No hay servicios ni categorías registrados. Crea una nueva categoría o
